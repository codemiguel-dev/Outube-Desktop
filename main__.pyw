from PyQt5.QtWidgets import QMainWindow, QLineEdit, QSizeGrip, QApplication, QMessageBox, QFileDialog
...
class Outube(QMainWindow):
    ...
    def select_download_folder(self):
        folder = QtWidgets.QFileDialog.getExistingDirectory(self, "Seleccionar Carpeta de Descarga")
        if folder:
            self.download_folder = folder
            self.download_status.setText(f"Carpeta seleccionada: {self.download_folder}")


    def descargar_mp3(self):
        url = self.urltxt.text()
        if not self.download_folder:
            self.download_status.setText("Por favor, selecciona una carpeta de descarga primero.")
            return
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                self.download_status.setText(f"El archivo MP3 se ha descargado: {mp3_filename}")
            end_time = time.time()  # Marca el final de la descarga
            download_time = end_time - start_time  # Calcula el tiempo total de descarga
            self.download_time.setText(f"Tiempo de descarga: {download_time:.2f} segundos")
